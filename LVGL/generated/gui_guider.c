...
{
	if (ui->pwd_buf_len > 20)
	{
		return;
	}
	ui->password_buf[ui->pwd_buf_len] = element;
	ui->pwd_buf_len++;
}
...
void remove_password_buf(lv_ui *ui)
{
	...
	ui->password_buf[ui->pwd_buf_len - 1] = '\0';
	ui->pwd_buf_len--;
}
...
void show_pwd_buf(lv_ui *ui)
{
	...
	while (ui->password_buf[i] != '\0')
	{
		lv_textarea_add_char(ui->screen_1_ta_1, ui->password_buf[i]);
		i++;
	}
}
